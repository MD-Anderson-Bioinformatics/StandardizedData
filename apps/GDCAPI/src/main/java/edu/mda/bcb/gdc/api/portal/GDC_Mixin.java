...
abstract public class GDC_Mixin
{
	...
	public ArrayList<String> getAvailableDatasets(File theDir, String theDatasetPrefix)
	{
		ArrayList<String> datasetDirs = new ArrayList<>();
		File[] files = theDir.listFiles(File::isDirectory);
		if (files!=null)
		{
			if (files.length>0)
			{
				for (File file : files)
				{
					if (file.getName().startsWith(theDatasetPrefix))
					{
						datasetDirs.add(file.getName());
					}
				}
			}
		}
	...
	public void processIndices(WorkflowData theWorkflowData, String theFirstString) throws Exception
	{
		...
		// if further specialization is neded, use an underscore "_" or after the second half string
		...
		ArrayList<String> datasetPrefixes = theWorkflowData.getDatasetName(theFirstString);
		for (String prefix : datasetPrefixes)
		{
			ArrayList<String> datasets = getAvailableDatasets(new File(new File(new File(new File(new File(mConvertDir, project), subProject), dataType), platform), version),
					prefix);
