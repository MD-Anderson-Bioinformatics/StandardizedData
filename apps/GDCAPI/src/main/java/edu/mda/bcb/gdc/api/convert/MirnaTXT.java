...
public class MirnaTXT
{
			...
			final File theDownloadDir, File theConvertDir, File theBiospecimenDir, File theClinicalDir, boolean theLegacyFlag) throws IOException, Exception
	{
		...
		{
			GDCAPI.printWarn("MirnaTXT No files to process");
			return;
		}
		...
		//// setup
		////////////////////////////////////////////////////////////////////////
		GDCAPI.printLn("MirnaTXT::processDirectory");
		...
		//// collect features
		////////////////////////////////////////////////////////////////////////
		GDCAPI.printLn("MirnaTXT collect features");
		...
		Arrays.asList(gdcArray)
				...
				.forEach((GDCFile loopFile) ->
				{
					...
					try
					{
						MirnaTXT.run(loopFile, features, theDownloadDir);
					}
					catch (Exception exp)
					{
						errors.add(exp);
		...
		//// process diseases
		////////////////////////////////////////////////////////////////////////
		GDCAPI.printLn("MirnaTXT process files");
		...
		Arrays.asList(gdcArray)
				...
				.forEach((GDCFile loopFile) ->
				{
					...
					if (0 == count % 100)
					{
						GDCAPI.printLn("MirnaTXT::processDirectory counter = " + count + " of " + gdcArray.length);
					}
					try
					{
						MirnaTXT.run(loopFile, newMatrix, theDownloadDir);
					}
					catch (Exception exp)
					{
						errors.add(exp);
		...
		{
			GDCAPI.printErr("MirnaTXT::processDirectory errors " + err.getMessage(), err);
		}
		try
		{
		...
		{
			GDCAPI.printErr("MirnaTXT::processDirectory exception caught" + exp.getMessage(), exp);
		}
	}
			...
			while(null!=line)
			{
				...
				if (!line.startsWith("#"))
				{
					...
					else 
					{
						...
						String feature = GDCAPI.getColumn("miRNA_ID", splitted, headers);
						String region = GDCAPI.getColumn("miRNA_region", splitted, headers);
						if (!"".equals(region))
						{
							if (region.startsWith("mature"))
							{
								String [] regSplit = region.split(",", -1);
								feature = feature + "|" + regSplit[1];
							}
							else
							{
						...
						feature = RnaseqTXT.cleanFeature(feature);
			...
			while(null!=line)
			{
				...
				if (!line.startsWith("#"))
				{
					...
					else 
					{
						...
						double value = Double.parseDouble(GDCAPI.getColumn("reads_per_million_miRNA_mapped", splitted, headers));
						feature = RnaseqTXT.cleanFeature(feature);
