 ...
 * @author Tod-Casasent
 */
public class HG38TranscriptMap extends IterateFile_Mixin
{
	public TreeSet<String> mValidChromosomes = null;
	
	public HG38TranscriptMap(TreeSet<String> theValidChromosomes, String theOutputFile)
	{
		super(theOutputFile, "unique\ttranscript-symbol\ttranscript-id\tgene-symbol\tensembl-id\tchromosome\tstart-loc\tend-loc\tstrand\tgene-type\ttranscript-type\tcalling-center", "#");
		mValidChromosomes = theValidChromosomes;
		System.out.println("HG38TranscriptMap");
	}
	...
	public void processLine(String [] theSplittedLine) throws Exception
	{
		...
		if ("transcript".equals(featureType))
		{
			...
			if (mValidChromosomes.contains(chromosome))
			{
				...
				//"unique\ttranscript-symbol\ttranscript-id\tgene-symbol\tensembl-id\tchromosome\tstart-loc\tend-loc\tstrand\tgene-type\ttranscript-type\tcalling-center"
				mOutputLines.add(geneSymbol + "|" + transcriptId + "\t" + transcriptName + "\t" + transcriptId + "\t" + geneSymbol + "\t" + ensemblId + "\t" + chromosome + "\t" + start + "\t" + stop + "\t" + strand + "\t" + geneType + "\t" + transcriptType + "\t" + center);
			}
		}
	}
}
