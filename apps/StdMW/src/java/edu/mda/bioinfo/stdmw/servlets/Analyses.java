 ...
 * @author Tod-Casasent
 */
@WebServlet(name = "analyses", urlPatterns =
{
	"/analyses"
})
public class Analyses extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		try
		{
			log("Servlet Analyses " + MWUrls.M_VERSION);
			...
			if (null!=summary)
			{
				TreeSet<Analysis> analysisSet = AnalysisUtil.getAnalysesForStudy(this.getServletContext(), request.getSession(), summary.study_id);
				...
				try (PrintWriter out = response.getWriter())
				{
					...
					out.print(gson.toJson(analysisSet));
				}
			}
			else
			{
			...
			log("Servlet Analyses returning");
		}
		catch(Exception exp)
		{
			log("Error getting list of analyses from MetabolomicsWorkbench", exp);
			throw new ServletException("Error getting list of analyses from MetabolomicsWorkbench", exp);
		}
	}
